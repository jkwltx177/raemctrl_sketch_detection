...
def calculate_feature_center(labels, feature_type, img_width, img_height, class_map):
    ...
    if len(feature_boxes) == 2 and feature_type in ['눈', '귀']:
        ...
        distance = math.sqrt((right_box[0] - left_box[0])**2 + (right_box[1] - right_box[1])**2)
...
# 특수 귀도리 오버레이 함수 (귀도리 전용)
def overlay_earmuffs(base_image, accessory_image, base_labels, class_map):
    """
    귀도리를 양쪽 귀를 연결하여 하나로 합성하는 함수
    귀도리는 양쪽 귀에 하나로 걸치는 방식으로 배치
    """
    ...
    if not ears_data or not ears_data[4] or not ears_data[5]:
        print("양쪽 귀를 찾을 수 없습니다. 귀도리 합성 불가.")
        return base_image, base_labels  # 양쪽 귀가 모두 없으면 원본 반환
    ...
    # 귀도리는 양쪽 귀를 모두 덮을 수 있어야 함
    scale_factor = 1.0  # 기본 스케일
    
    # 크기 계산 - 두 귀 사이 거리의 약 1.2배로 설정
    # (양쪽 귀를 모두 커버하고 약간의 여유 필요)
    ...
    # 약간 위로 조정 (귀도리는 보통 귀 위쪽으로 약간 올라감)
    offset_y = -final_height * 0.1
    
    paste_x = int(center_x - final_width/2)
    paste_y = int(center_y - final_height/2 + offset_y)
...
if __name__ == "__main__":
    ...
    # 새로운 귀도리 합성 함수로 테스트
    result_img, result_labels = overlay_earmuffs(
