...
# 바운딩 박스 생성 전 유효성 검증 및 YOLO 형식 라벨 생성
def validate_and_create_bbox(paste_x, paste_y, roi_width, roi_height, img_width, img_height, class_id):
    """
    바운딩 박스 생성 전 유효성 검증 및 YOLO 형식 라벨 생성
    """
    # 중심점 계산
    x_center = paste_x + roi_width/2
    y_center = paste_y + roi_height/2
    
    # 검증: 바운딩 박스가 이미지 범위 내에 있는지 확인
    if (0 <= x_center <= img_width and 
        0 <= y_center <= img_height and
        roi_width > 0 and roi_height > 0):
        
        # 바운딩 박스가 유효하면 YOLO 형식 라벨 생성
        label = create_yolo_annotation(
            class_id,
            x_center,
            y_center,
            roi_width,
...
# 액세서리 방향 감지 및 자동 회전 함수 (개선된 버전 3)
def detect_and_fix_accessory_orientation_improved_v3(accessory_image, accessory_class):
    """
    액세서리 이미지의 방향을 감지하고 필요시 자동으로 회전합니다.
    개선된 버전: 회전 후 크기 항상 업데이트
    """
    # PIL 이미지로 변환
    ...
    if should_rotate:
        ...
        print(f"  회전 전 크기: {width}x{height}, 회전 후 크기: {new_width}x{new_height}")
...
# 귀도리 오버레이 함수 (개선된 버전 3)
def overlay_earmuffs_improved_v3(base_image, accessory_image, base_labels, class_map):
    """
    귀도리를 양쪽 귀를 연결하여 하나로 합성하는 함수 (개선된 버전 3)
    - 크기 조정과 바운딩 박스 계산 일치성 향상
    """
    ...
    accessory_pil, was_rotated, rotation_angle = detect_and_fix_accessory_orientation_improved_v3(accessory_pil, '귀도리')
    
    # 얼굴 각도 기반 추가 회전 (얼굴이 기울어져 있는 경우)
    if abs(face_angle) > 5:  # 얼굴이 5도 이상 기울어진 경우 회전
        print(f"얼굴 각도 {face_angle:.1f}도 감지 - 회전 적용")
        accessory_pil = accessory_pil.rotate(face_angle, expand=True, resample=Image.BICUBIC)
        # 회전 후 크기 업데이트
        rotated_width, rotated_height = accessory_pil.size
        print(f"  회전 후 크기 업데이트: {rotated_width}x{rotated_height}")
    ...
    # 귀도리 위치: 귀 위쪽으로 배치
    ear_top_y = min(left_y, right_y) - ears_data[3]/2
    ...
    # 실제 합성될 영역 계산
    roi_width = min(new_width, img_width - paste_x)
    roi_height = min(new_height, img_height - paste_y)
    ...
    # 바운딩 박스 생성 - 실제 합성된 영역과 정확히 일치하도록
    earmuffs_x_center = paste_x + roi_width/2
    earmuffs_y_center = paste_y + roi_height/2
    
    # 검증: 바운딩 박스가 이미지 범위 내에 있는지 확인
    if (0 <= earmuffs_x_center <= img_width and 
        0 <= earmuffs_y_center <= img_height and
        roi_width > 0 and roi_height > 0):
        ...
        # 디버깅 정보 출력
        print(f"귀도리 합성 완료: 위치({paste_x},{paste_y}), 크기({roi_width}x{roi_height})")
    else:
        print("경고: 귀도리의 바운딩 박스가 이미지 범위를 벗어납니다. 라벨 생성 취소.")
    
    return result_image, updated_labels

# 대칭 액세서리 오버레이 함수 (개선된 버전)
def overlay_symmetric_accessory_improved(base_image, accessory_image, accessory_class, target_type, base_labels, class_map):
    """
    대칭적인 액세서리를 양쪽에 배치하는 함수 (개선된 버전)
    - 크기 조정과 바운딩 박스 계산 일치성 향상
    """
    # 귀도리인 경우 특수 처리 함수로 위임
    if accessory_class == '귀도리':
        return overlay_earmuffs_improved_v3(base_image, accessory_image, base_labels, class_map)
    ...
    # 액세서리 방향 감지 및 자동 회전 (개선된 버전 사용)
    accessory_pil, was_rotated, rotation_angle = detect_and_fix_accessory_orientation_improved_v3(accessory_pil, accessory_class)
    ...
    if abs(face_angle) > 10:  # 얼굴이 10도 이상 기울어진 경우에만 회전
        ...
        # 회전 후 크기 명시적 업데이트
        new_width, new_height = left_accessory_pil.size
        print(f"  왼쪽 회전 적용 후 크기: {new_width}x{new_height}")
    ...
    # 왼쪽 이미지 경계 처리 - 단일 단계로 통합
    left_paste_x = max(0, min(left_paste_x, img_width - left_width))
    left_paste_y = max(0, min(left_paste_y, img_height - left_height))
    
    # 왼쪽 실제 합성될 영역 계산
    left_roi_width = min(left_width, img_width - left_paste_x)
    left_roi_height = min(left_height, img_height - left_paste_y)
    ...
    # 오른쪽용 크기 조절
    right_accessory_pil = right_accessory_pil.resize((new_width, new_height), Image.LANCZOS)
    ...
    if abs(face_angle) > 10:
        ...
        print(f"  오른쪽 회전 적용 후 크기: {right_width}x{right_height}")
    else:
        right_width, right_height = right_accessory_pil.size
    ...
    # 오른쪽 이미지 경계 처리 - 단일 단계로 통합
    right_paste_x = max(0, min(right_paste_x, img_width - right_width))
    right_paste_y = max(0, min(right_paste_y, img_height - right_height))
    
    # 오른쪽 실제 합성될 영역 계산
    right_roi_width = min(right_width, img_width - right_paste_x)
    right_roi_height = min(right_height, img_height - right_paste_y)
    ...
    # 양쪽 액세서리의 바운딩 박스 생성 및 검증
    accessory_class_id = class_map[accessory_class]
    
    # 왼쪽 바운딩 박스 검증 및 생성
    is_valid_left, left_label = validate_and_create_bbox(
        left_paste_x, left_paste_y, left_roi_width, left_roi_height, 
        img_width, img_height, accessory_class_id
    )
    
    # 오른쪽 바운딩 박스 검증 및 생성
    is_valid_right, right_label = validate_and_create_bbox(
        right_paste_x, right_paste_y, right_roi_width, right_roi_height, 
        img_width, img_height, accessory_class_id
    )
    
    # 라벨 추가 (유효한 경우에만)
    if is_valid_left:
        updated_labels.append(left_label)
        print(f"왼쪽 {accessory_class} 합성 완료: 위치({left_paste_x},{left_paste_y}), 크기({left_roi_width}x{left_roi_height})")
    else:
        print(f"경고: 왼쪽 {accessory_class}의 바운딩 박스가 이미지 범위를 벗어납니다. 라벨 생성 취소.")
    
    if is_valid_right:
        updated_labels.append(right_label)
        print(f"오른쪽 {accessory_class} 합성 완료: 위치({right_paste_x},{right_paste_y}), 크기({right_roi_width}x{right_roi_height})")
    else:
        print(f"경고: 오른쪽 {accessory_class}의 바운딩 박스가 이미지 범위를 벗어납니다. 라벨 생성 취소.")
    
    return result_image, updated_labels

# 안경 오버레이 함수 (개선된 버전)
def overlay_glasses_improved(base_image, accessory_image, base_labels, class_map):
    """
    안경을 눈에 맞게 배치하는 특화 함수 (개선된 버전)
    - 크기 조정과 바운딩 박스 계산 일치성 향상
    """
    result_image = base_image.copy()
    img_height, img_width = result_image.shape[:2]
    ...
    if not eyes_data or not eyes_data[4] or not eyes_data[5]:
        print("양쪽 눈을 찾을 수 없습니다. 안경 합성 불가.")
    ...
    # 액세서리 방향 감지 및 자동 회전 (개선된 버전 사용)
    accessory_pil, was_rotated, rotation_angle = detect_and_fix_accessory_orientation_improved_v3(accessory_pil, '안경')
    
    # 안경 크기 조절
    scale_factor = eye_distance / (accessory_pil.width * 0.5)  # 안경 크기를 적절하게 조정
    ...
    # 바운딩 박스 생성 - 실제 합성된 영역과 정확히 일치하도록
    glasses_x_center = paste_x + roi_width/2
    glasses_y_center = paste_y + roi_height/2
    
    # 검증: 바운딩 박스가 이미지 범위 내에 있는지 확인
    if (0 <= glasses_x_center <= img_width and 
        0 <= glasses_y_center <= img_height and
        roi_width > 0 and roi_height > 0):
        
        # YOLO 형식으로 새 라벨 생성
        glasses_class_id = class_map['안경']
        glasses_label = create_yolo_annotation(
            glasses_class_id,
            glasses_x_center,
            glasses_y_center,
            roi_width,
        ...
        # 라벨 추가
        updated_labels.append(glasses_label)
        
        # 디버깅 정보 출력
        print(f"안경 합성 완료: 위치({paste_x},{paste_y}), 크기({roi_width}x{roi_height})")
    else:
        print("경고: 안경의 바운딩 박스가 이미지 범위를 벗어납니다. 라벨 생성 취소.")
    
    return result_image, updated_labels

# 액세서리 오버레이 함수 (일반 액세서리용 - 개선된 버전)
def overlay_accessory_improved_v2(base_image, accessory_image, accessory_class, target_type, base_labels, class_map):
    """
    개선된 액세서리 오버레이 함수 - 크기 조정과 바운딩 박스 계산 일치성 향상
    """
    ...
    if not target_part_boxes:
        print(f"대상 부위({target_type})를 찾을 수 없습니다. 합성 불가.")
    ...
    # 개선: 회전 후에 크기를 항상 업데이트
    curr_width, curr_height = accessory_pil.size
    
    # 크기 계산 - 대상 부위의 크기에 맞게 액세서리 크기 조절
    ...
    # 변환과 회전 적용 (추가 회전이 필요한 경우)
    if rotation != 0:
        print(f"{accessory_class} 추가 회전 적용: {rotation}도")
        accessory_pil = accessory_pil.rotate(rotation, expand=True, resample=Image.BICUBIC)
        # 개선: 회전 후 크기 명시적 업데이트
        new_width, new_height = accessory_pil.size
        print(f"  회전 후 크기 업데이트: {new_width}x{new_height}")
    ...
    # 합성 좌표 계산
    paste_x = int(x_center - new_width/2 + offset_x)
    paste_y = int(y_center - new_height/2 + offset_y)
    ...
    # 바운딩 박스 생성 - 실제 합성된 영역과 정확히 일치하도록
    accessory_x_center = paste_x + roi_width/2
    accessory_y_center = paste_y + roi_height/2
    
    # 검증: 바운딩 박스가 이미지 범위 내에 있는지 확인
    if (0 <= accessory_x_center <= img_width and 
        0 <= accessory_y_center <= img_height and
        roi_width > 0 and roi_height > 0):
        ...
        # 디버깅 정보 출력
        print(f"{accessory_class} 합성 완료: 위치({paste_x},{paste_y}), 크기({roi_width}x{roi_height})")
    else:
        print(f"경고: {accessory_class}의 바운딩 박스가 이미지 범위를 벗어납니다. 라벨 생성 취소.")
    
    return result_image, updated_labels

# 눈썹 출현 빈도를 대폭 높이기 위한 개선된 데이터셋 생성 함수
def generate_improved_composited_dataset_v3(
    base_images_dir,
    accessories_dir,
    output_dir,
    class_map,
    ...
    for i in range(num_combinations):
        ...
        if use_eyebrows and '눈썹' in accessories and accessories['눈썹']:
            ...
            try:
                ...
                result_img, result_labels = overlay_symmetric_accessory_improved(
        ...
        if use_ribbon and '리본' in accessories and accessories['리본']:
            ...
            # 개선된 함수 사용
            result_img, result_labels = overlay_accessory_improved_v2(
            ...
            if target_type:
                try:
                    ...
                    # 귀도리 특별 처리 (개선된 함수 사용)
                        ...
                        result_img, result_labels = overlay_earmuffs_improved_v3(
                            result_img, accessory_img, result_labels, class_map
                        )
                    # 안경 특별 처리 (개선된 함수 사용)
                    elif acc_class == '안경':
                        result_img, result_labels = overlay_glasses_improved(
                            result_img, accessory_img, result_labels, class_map
                        )
                    # 일반 액세서리 처리 (개선된 함수 사용)
                    else:
                        result_img, result_labels = overlay_accessory_improved_v2(
...
def test_specific_image():
    ...
    # 귀도리 합성 테스트 - 개선된 함수 사용
    result_img, result_labels = overlay_earmuffs_improved_v3(base_img, accessory_img, base_labels, class_map)
...
if __name__ == "__main__":
    ...
    # 특정 이미지 테스트 또는 데이터셋 생성 실행
    test_specific_image_bool = False
    if test_specific_image_bool:
        test_specific_image()
    else:
        # 정상 데이터셋 생성 실행 - 개선된 함수 사용
        generate_improved_composited_dataset_v3(
            base_images_dir="C:/capstone/data/validation/images",
            accessories_dir="C:/capstone/data/validation/accessories_png",
            output_dir="C:/capstone/data/validation/composited_final",
            class_map=class_map,
            accessory_target_map=accessory_target_map,
            num_combinations=4000,  # 생성할 이미지 수
            eyebrow_probability=0.8,   # 눈썹 출현 빈도 (0.0~1.0)
            ribbon_probability=0.2,
