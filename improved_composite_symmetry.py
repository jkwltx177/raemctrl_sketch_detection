...
def is_full_person_image(image_path):
    ...
    # s_1003, s_1004로 시작하는 이미지
    # if filename.startswith('s_1003') or filename.startswith('s_1004'):
    #     return True
    ...
    # 일반적인 사람 이미지도 체크하기 위해 YOLO 라벨을 확인
    # try:
    #     label_path = os.path.splitext(image_path)[0].replace("images", "labels") + ".txt"
    #     if os.path.exists(label_path):
    #         with open(label_path, 'r') as f:
    #             labels = f.readlines()
    #             for label in labels:
    #                 parts = label.strip().split()
    #                 if parts and int(parts[0]) == 0:  # 클래스 ID 0이 '사람전체'인 경우
    #                     return True
    # except Exception as e:
    #     print(f"라벨 파일 확인 중 오류: {e}")
...
def detect_accessory_orientation(accessory_image):
    ...
    # 객체의 방향을 추정하기 위한 스피드 향상을 위한 중심 모멘트 계산
    ...
    if isinstance(accessory_image, Image.Image) and hasattr(accessory_image, 'filename'):
        ...
        if 's_0794_24573_620913' in filename:
            ...
            axis_ratio = 2.0  # 가로로 누워있는 상태
    
    # 이미지 작업 위해 선택적으로 출력 확인
...
def overlay_symmetric_accessory(base_image, accessory_image, accessory_class, target_type, base_labels, class_map):
    ...
    # 합성 좌표 계산 (왼쪽)
    left_paste_x = int(left_x - left_accessory_pil.width/2)
    left_paste_y = int(left_y + offset_y)
    
    # 이미지가 경계를 벗어나지 않도록 조정
    left_paste_x = max(0, min(left_paste_x, img_width - left_accessory_pil.width))
    left_paste_y = max(0, min(left_paste_y, img_height - left_accessory_pil.height))
    ...
    right_paste_x = int(right_x - right_accessory_pil.width/2)
    right_paste_y = int(right_y + offset_y)
    
    # 이미지가 경계를 벗어나지 않도록 조정
    right_paste_x = max(0, min(right_paste_x, img_width - right_accessory_pil.width))
    right_paste_y = max(0, min(right_paste_y, img_height - right_accessory_pil.height))
